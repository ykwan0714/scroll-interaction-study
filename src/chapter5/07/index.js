...
;(() => {
  ...
  <section id="First" class="section_first">
  ...
  <section id="Mask" class="section_mask">
  ...
  window.addEventListener('load', () => {
    const sperateEvent = (query, callback) => {
      ...
      const calculateActivation = () => {
        ...
        if (
          winScrollY >= sectionTop &&
          winScrollY <= sectionBottom &&
          isActive === false
        ) {
          isActive = true
    ...
    const anchors = $('.nav .item_nav a')
    anchors.forEach((anchor) => {
      anchor.addEventListener(
        'click',
        (e) => {
          e.preventDefault()
          const section = $(anchor.hash)
          if (section) {
            const offTop = section.offsetTop + window.scrollY
            scrollAnimationTo(offTop)
          }
        },
        false
      )
